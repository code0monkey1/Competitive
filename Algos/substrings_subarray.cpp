 ...
 */
     ...
     void printing_substring_subarray(const auto & s){ // can substitute string sinsted of this 

     	for(int i=0;i<s.size();i++){ //start position

     		for(int j=i;j<s.size();j++){ // end position 

     			    for(int k=i;k<=j;k++){  // printing everything between start and end 
     			    	
     			    	 cout<<s[k]; //individual  elements generated here 
     			    	}
