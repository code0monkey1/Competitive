...
int  lcm(int  num,auto & v, int size){

	if(size==v.size())return num;

	return lcm((num*v[size])/__gcd(num,v[size]), v, size+1);
}
int totGcd(int num,auto &v ,int size){

	if(size==v.size())return num;

	return totGcd(__gcd(num,v[size]),v,size+1);
}
...
int main(){
	...
	if(totGcd(vb[0],vb,0)!=1 && lcm(va[0],va,0)<=totGcd(vb[0],vb,0)){

		int temp=lcm(va[0],va,0);

		while(temp<=*min_element(all(vb))){
			if(totGcd(vb[0],vb,0)%temp==0 && temp%lcm(va[0],va,0)==0){
