...
void countSort(vll & v, ll multi){
	...
	re(i,n)index[(v[i]/multi)%10]++;
	//debug(v,index);
	for(int i=1;i<10;i++)index[i]+=index[i-1];

		
		for(int i=n-1;i>=0;i--){
			output[index[(v[i]/multi)%10]-1]=v[i];
			index[(v[i]/multi)%10]--;
		}

		re(i,n)cout<<output[i]<<" ";
		v=output;nl;

	}

	void radixSort(vll & v,ll maxx){
		int multi=1;
	...
	int main(){fastio
		...
		FILE_IO
	#endif
		vll v={170 ,45, 75, 90 ,802 ,24, 2 ,66};
