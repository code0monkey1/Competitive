...
typedef set<str> ss;
typedef set<int> si;
typedef set<unsigned ll> sull;
typedef multiset <unsigned ll> msull;
...
ull recurring_gcd(int lhs,int rhs,int sum){

	if(lhs==rhs)return gcd(sum,v[rhs]);

	return recurring_gcd(lhs+1,rhs,gcd(sum,v[rhs]));
...
int main() {
	...
	re(i,n){
		...
		for(int j=i;j<n;j++){
			...
			s.insert(recurring_gcd(i,j,v[i]));
